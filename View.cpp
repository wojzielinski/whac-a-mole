...
void View::draw_playing(int weapon){
    ...
    WIN.draw(label.get_label("HP: " + std::to_string(int(PLAYER.get_hp())),45,0.4,0.16));
    WIN.draw(label.get_label("Stamina: " + std::to_string(int(PLAYER.get_stamina())),45,0.6,0.16));
    WIN.display();
};
...
void View::draw_leaderboard(){
    ...
    WIN.draw(label.get_label("Leaderboard",120,palette.get_color(BG),0.5,0.1));
